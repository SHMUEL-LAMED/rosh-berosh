...
export function LoginScreen() {
  ...
  return <main className="login-shell" dir="rtl"><section className="login-card"><img className="login-logo" src="/badge.jpg" alt="מצעד האלבומים · 25 שנות מוזיקה" /><p className="kicker">ראש בראש</p><h1>מתחברים ומצביעים</h1><p>מצעד 25 שנות מוזיקה יהודית: בוחרים את האלבומים, השירים והזמרים האהובים עליכם. התוצאות וההצבעה נשמרות לפי כללי המצעד.</p><p>כדי לשמור על הצבעה הוגנת, הכניסה מתבצעת באמצעות חשבון Google.</p><div ref={button} className="google-button" />{error && <p className="vote-error">{error}</p>}<small>לא נפרסם דבר בחשבון שלכם ולא נקבל את הסיסמה שלכם.</small><p><a href="https://shmuel-lamed.github.io/rosh-berosh-2/">להאזנה לתוכניות ראש בראש ולארכיון ההקלטות</a></p></section></main>;
}

...
export default function Home() {
  ...
  return <main className={`voting-shell ${player ? "with-player" : ""}`} dir="rtl">
    ...
    {voted ? <section className="vote-card"><div className="empty-catalog"><p className="kicker">ההצבעה כבר נקלטה</p><h2>כבר הצבעתם בסקר הזה</h2><p>עדיין אפשר להאזין לכל השירים, ומנהלים יכולים להיכנס למערכת הניהול מהכפתור למעלה.</p><SubscribeCard /></div></section> : catalog && !catalog.rules.votingOpen ? <section className="vote-card"><div className="empty-catalog"><h2>ההצבעה סגורה כרגע</h2><p>מנהל המצעד יפתח אותה בקרוב.</p></div></section> : <>

...
type Overview = { albums: Album[]; songs: Song[]; artists: Artist[]; managers: string[]; yemotConnected: boolean; votes: { total?: number; phone?: number; site?: number }; settings: Settings; ivrPrompts: IvrPrompt[]; results: { albums: Result[]; songs: Result[]; artists: Result[] } };
type Tab = "dashboard" | "albums" | "artists" | "ivr" | "settings" | "managers" | "results";
...
export default function AdminPage() {
  ...
  const deleteCover = async (albumId: string) => {
  ...
  return <main className="admin-shell" dir="rtl">
    <aside className="admin-side"><div className="vote-header"><div className="logo-mark">ר</div><div><strong>ראש בראש</strong><small>מערכת ניהול</small></div></div><nav>
      <Nav active={tab === "dashboard"} onClick={() => setTab("dashboard")}>סקירה כללית</Nav><Nav active={tab === "albums"} onClick={() => setTab("albums")}>אלבומים ושירים</Nav><Nav active={tab === "artists"} onClick={() => setTab("artists")}>זמרים</Nav><Nav active={tab === "ivr"} onClick={() => setTab("ivr")}>קריינות לקו</Nav><Nav active={tab === "settings"} onClick={() => setTab("settings")}>הגדרות הסקר</Nav><Nav active={tab === "managers"} onClick={() => setTab("managers")}>מנהלי המערכת</Nav><Nav active={tab === "results"} onClick={() => setTab("results")}>תוצאות</Nav><Link href="/">מעבר לאתר</Link>
    </nav><button className="admin-logout" onClick={logout}>יציאה מהחשבון</button></aside>
    <section className="admin-main"><header><div><p className="kicker">שלום, {user.name}</p><h1>{tab === "dashboard" ? "מרכז הניהול" : ({ albums: "אלבומים ושירים", artists: "זמרים", ivr: "קריינות לקו", settings: "הגדרות הסקר", managers: "מנהלי המערכת", results: "תוצאות" } as Record<string, string>)[tab]}</h1></div><span>{user.picture && <img src={user.picture} alt="" />}{user.email}</span></header>
...
function AlbumEditor({ album, songs, onSave, onToggle, onDelete, onDeleteCover, onFiles }: { album: Album; songs: Song[]; onSave(event: FormEvent<HTMLFormElement>, kind: "album" | "song" | "artist"): void; onToggle(kind: "album" | "song" | "artist", id: string, active: boolean): void; onDelete(kind: "album" | "song" | "artist", id: string): void; onDeleteCover(id: string): void; onFiles(files: FileList | null): void }) {
  const [open, setOpen] = useState(false);
  return <article className="album-editor"><header onClick={() => setOpen(!open)}>{album.coverUrl ? <img src={album.coverUrl} alt="" /> : <i>♫</i>}<div><b>{album.title}</b><small>{album.artistName} · {songs.length} שירים</small></div><span>{open ? "⌃" : "⌄"}</span></header>{open && <div className="album-editor-body"><form onSubmit={(event) => onSave(event, "album")}><input type="hidden" name="id" value={album.id} /><input name="title" defaultValue={album.title} /><input name="artistName" defaultValue={album.artistName} /><input name="position" type="number" defaultValue={album.position} /><button>שמירת אלבום</button></form><div className="album-tools"><label className="file-field">החלפת עטיפה / הוספת שירים<input type="file" multiple accept="audio/*,image/*" onChange={(event) => onFiles(event.target.files)} /></label><Toggle active={!!album.active} onClick={() => onToggle("album", album.id, !album.active)} />{album.coverUrl && <button type="button" onClick={() => onDeleteCover(album.id)}>הסרת עטיפה</button>}<button type="button" className="danger" onClick={() => onDelete("album", album.id)}>מחיקת אלבום מלא</button></div><div className="song-admin-list">{songs.map((song) => <SongEditor key={song.id} song={song} onSave={onSave} onToggle={onToggle} onDelete={onDelete} />)}</div></div>}</article>;
}
...
function Results({ data }: { data: Overview["results"] }) { const [kind, setKind] = useState<keyof Overview["results"]>("albums"); const list = data[kind]; const download = () => { const heading = kind === "albums" ? "אלבומים" : kind === "songs" ? "שירים" : "זמרים"; const q = (value: unknown) => `"${String(value ?? "").replaceAll('"', '""')}"`; const rows = [["מקום", heading, "אלבום", "קולות"], ...list.map((item, index) => [index + 1, item.title || item.name || "", item.albumTitle || "", item.votes])]; const blob = new Blob(["\ufeff" + rows.map((row) => row.map(q).join(",")).join("\n")], { type: "text/csv;charset=utf-8" }); const link = document.createElement("a"); link.href = URL.createObjectURL(blob); link.download = `rosh-berosh-${kind}.csv`; link.click(); URL.revokeObjectURL(link.href); }; return <AdminSection title="תוצאות בזמן אמת"><div className="result-tabs"><button onClick={() => setKind("albums")}>אלבומים</button><button onClick={() => setKind("songs")}>שירים</button><button onClick={() => setKind("artists")}>זמרים</button><button className="export-results" onClick={download}>הורדה לאקסל</button></div><p className="panel-help">קובץ CSV תואם Excel, עם הדירוג והקולות.</p><div className="results-table">{list.map((item, index) => <div key={item.id}><b>{index + 1}</b><span>{item.title || item.name}<small>{item.albumTitle}</small></span><strong>{item.votes} קולות</strong></div>)}</div></AdminSection>; }

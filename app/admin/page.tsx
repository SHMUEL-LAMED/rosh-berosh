...
type PollArchive = { key: string; name: string; createdAt: number; votes: number; albums: number; songs: number; artists: number; programs?: number | null };
...
export default function AdminPage() {
  ...
  const addArtist = async (event: FormEvent<HTMLFormElement>) => {
    ...
    notify("שומרים…");
    try {
      const { id } = await api("/api/admin/catalog", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ kind: "artist", name, imageUrl: String(values.get("imageUrl") || ""), position: String(values.get("position") || "") }) });
  ...
  const moveItem = async (kind: "album" | "artist", index: number, direction: -1 | 1) => {
    ...
    if (kind === "album") {
      ...
      try { await api("/api/admin/reorder", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ kind: "album", ids: next.map((a) => a.id) }) }); }
      catch (error) { setAlbumOrder(null); notify(error instanceof Error ? error.message : "שינוי הסדר נכשל."); }
    } else {
      ...
      try { await api("/api/admin/reorder", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ kind: "artist", ids: next.map((a) => a.id) }) }); }
      catch (error) { setArtistOrder(null); notify(error instanceof Error ? error.message : "שינוי הסדר נכשל."); }
    }
  };
...
function RecorderAccessPanel({ recorders, onSaved, onMessage }: { recorders: string[]; onSaved(): Promise<void> | void; onMessage(message: string): void }) {
  ...
  return <div className="recorder-access">
    <p className="panel-help">רק מספרים שמופיעים כאן יוכלו להיכנס לקו ההקלטות והניהול. הזיהוי אוטומטי לפי מספר המתקשר ואין צורך בקוד.</p>
...
function SettingsPanel({ data, onSaved }: { data: Overview; onSaved(): void }) {
  ...
  const clearPoll = async () => { if (!confirm("למחוק את כל הסקר הפעיל? לפני המחיקה יישמר בארכיון גיבוי מלא כולל קובצי המדיה, ויירד למחשב קובץ נתונים JSON.")) return; const backup = await fetch("/api/admin/backup"); if (!backup.ok) return notify("לא הצלחנו ליצור גיבוי ולכן הסקר לא נמחק."); const blob = await backup.blob(), link = document.createElement("a"); link.href = URL.createObjectURL(blob); link.download = `rosh-berosh-backup-${Date.now()}.json`; link.click(); URL.revokeObjectURL(link.href); const response = await fetch("/api/admin/poll?skipArchive=1", { method: "DELETE" }); if (response.ok) { notify("גיבוי מלא נשמר בארכיון, קובץ הנתונים ירד למחשב ונפתחה טיוטה חדשה."); onSaved(); } else notify("מחיקת הסקר נכשלה."); };
...
function AlbumEditor({ album, songs, onSave, onToggle, onDelete, onFiles, onUploadCover, onRemoveCover, onReordered, onMessage }: { album: Album; songs: Song[]; onSave(event: FormEvent<HTMLFormElement>, kind: "album" | "song" | "artist"): void; onToggle(kind: "album" | "song" | "artist", id: string, active: boolean): void; onDelete(kind: "album" | "song" | "artist", id: string): void; onFiles(files: FileList | null): void; onUploadCover(albumId: string, file: File): Promise<void> | void; onRemoveCover(albumId: string): Promise<void> | void; onReordered(): Promise<void> | void; onMessage(message: string): void }) {
  const [open, setOpen] = useState(false);
  const [busy, setBusy] = useState(false);
  // הסדר המקומי מזוהה לפי קבוצת השירים ולא לפי זהות המערך, שנבנה מחדש בכל רנדור של דף הניהול.
  const songSignature = [...songs].map((song) => song.id).sort().join(",");
  const [songOverride, setSongOverride] = useState<{ signature: string; order: Song[] } | null>(null);
  const songOrder = songOverride && songOverride.signature === songSignature ? songOverride.order : songs;
  const moveSong = async (index: number, direction: -1 | 1) => {
    const target = index + direction;
    if (target < 0 || target >= songOrder.length) return;
    const previous = songOrder;
    const next = [...songOrder]; [next[index], next[target]] = [next[target], next[index]]; setSongOverride({ signature: songSignature, order: next });
    try {
      const response = await fetch("/api/admin/reorder", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ kind: "song", ids: next.map((s) => s.id) }) });
      const result = await response.json().catch(() => null);
      if (!response.ok) throw new Error(result?.error || "שינוי סדר השירים נכשל.");
      onMessage("סדר השירים נשמר.");
      await onReordered();
    } catch (error) {
      setSongOverride({ signature: songSignature, order: previous });
      onMessage(error instanceof Error ? error.message : "שינוי סדר השירים נכשל.");
    }
  };
  // תמונת האלבום היא התמונה של כל השירים שבו. רק לאלבום בלי תמונה מוצגת
  // העטיפה שנשלפה מאחד השירים.
  const uniqueCovers = album.coverUrl ? [album.coverUrl] : [...new Set(songs.map((s) => s.coverUrl).filter(Boolean))];
...
function ManagersPanel({ managers, fixed, currentEmail, onSaved, onMessage }: { managers: string[]; fixed: string[]; currentEmail: string; onSaved(): Promise<void> | void; onMessage(message: string): void }) {
  ...
  return <AdminSection title="הרשאות מנהלים"><p className="panel-help">הוסיפו כתובת Google. משתמש רגיל לא יראה את מערכת הניהול; לחשבון שמופיע כאן יופיע כפתור ניהול לאחר הכניסה. זו אותה רשימה גם לאתר התוכניות — מי שמנהל כאן מנהל גם שם, והתנתקות במקום אחד מנתקת משניהם. מנהל קבוע מוגדר בהגדרות השרת, ואי אפשר להסיר אותו מכאן.</p><form onSubmit={add}><input name="email" type="email" placeholder="כתובת Gmail או Google Workspace" required /><button>הוספת מנהל</button></form><div className="admin-list manager-list">{managers.map((email) => { const you = email === currentEmail, permanent = fixed.includes(email); return <article key={email}><div><i>מ</i><span><b>{email}</b><small>{you && permanent ? "החשבון הנוכחי · מנהל קבוע" : you ? "החשבון הנוכחי" : permanent ? "מנהל קבוע — אי אפשר להסיר מכאן" : "מנהל מערכת"}</small></span></div><button type="button" className="danger" disabled={you || permanent} title={permanent ? "מנהל קבוע מוגדר בהגדרות השרת" : undefined} onClick={() => remove(email)}>הסרת הרשאה</button></article>; })}</div></AdminSection>;
}
...
function ArchivesPanel({ onChanged, onMessage }: { onChanged(): Promise<void> | void; onMessage(message: string): void }) {
  ...
  return <AdminSection title="ארכיון סקרים וגיבויים"><p className="panel-help">שמרו גרסה מלאה של הסקר הנוכחי, כולל קובצי השמע והתמונות. אפשר לשחזר כל סקר כטיוטה, בלי לפרסם אותו מיד. הגיבוי כולל גם את אתר התוכניות (התוכניות, העונות וההודעות), ושחזור מחזיר את שניהם.</p><form className="archive-form" onSubmit={archive}><input name="name" placeholder="שם הסקר בארכיון" /><button>שמירת הסקר הנוכחי בארכיון</button></form>{loading ? <p>טוען ארכיון…</p> : <div className="archive-list">{archives.length ? archives.map((item) => <article key={item.key}><div><b>{item.name}</b><small>{new Date(item.createdAt).toLocaleString("he-IL")} · {item.votes} הצבעות</small><span>{item.albums} אלבומים · {item.songs} שירים · {item.artists} זמרים{item.programs != null && <> · {item.programs} תוכניות</>}</span></div><div><button onClick={() => restore(item)}>שחזור כטיוטה</button><button className="danger" onClick={() => remove(item)}>מחיקה מהארכיון</button></div></article>) : <p>עדיין אין סקרים בארכיון.</p>}</div>}</AdminSection>;
}
